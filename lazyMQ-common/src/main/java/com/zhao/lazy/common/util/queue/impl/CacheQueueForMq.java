 ...
 * 功能描述：
 */
public class CacheQueueForMq<E> implements ZFifoQueue<E>{

	private Object[] datas ;
	private int size = 0;
	private final int DEFAULT_SIZE = 10;
	private Set<String> messageBeanKey = new HashSet<String>();
	public CacheQueueForMq() {
		checkNull();
	}
	...
	public E pop() {
		synchronized (datas) {
			...
			Object model = null;
			model = datas[0];
			this.messageBeanKey.remove(model.toString());
	...
	public List<E> popList(int popSize) {
		synchronized (datas) {
			...
			if(size < popSize) {
				...
				size = 0;
				messageBeanKey = new HashSet<String>();
	...
	public boolean flush(E model) {
		synchronized (datas) {
			if(this.messageBeanKey.contains(model.toString())) {
				return false;
			}
			this.messageBeanKey.add(model.toString());
			checkNull();
			checkDataSize();
			datas[size++] = model;
		}
